...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		for (i in 0...50){
		var fruna:Array<String> = ["FRUNA CATOINGA", "TOINGA", "TOINGA"];
		trace(fruna[i] + "\n");
		}
	...
	function startIntro()
	{
		...
		var path = "mods/" + Paths.currentModDirectory + "/images/pressText.png";
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "mods/images/pressText.png";
		}
		//trace(path, FileSystem.exists(path));
		if (!FileSystem.exists(path)){
			path = "assets/images/pressText.png";
		}
		//trace(path, FileSystem.exists(path));
		titleText.frames = FlxAtlasFrames.fromSparrow(BitmapData.fromFile(path),File.getContent(StringTools.replace(path,".png",".xml")));
		#else

		titleText.frames = Paths.getSparrowAtlas('pressText');
		#end
		...
		if (animFrames.length > 0) {
			...
			titleText.animation.addByPrefix('idle', "loop", 24);
			titleText.animation.addByPrefix('press', "pressed", 24, false);
		}
		else {
			newTitle = false;
		...
		blackScreen.loadGraphic(Paths.image("bg"));
		// bg.antialiasing = ClientPrefs.globalAntialiasing;
		// bg.setGraphicSize(Std.int(bg.width * 0.6));
		// bg.updateHitbox();
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				if(titleText != null){ 
					...
					titleText.animation.curAnim.curFrame = 1;
				}
